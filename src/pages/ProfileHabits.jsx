...
export default function ProfileHabits() {
  ...
  const loadAll = async () => {
    ...
    const hRes = await fetch("http://localhost:5003/habits", {
    ...
    const all = await Promise.all(
      hData.map((h) =>
        fetch(`http://localhost:5003/habits/${h._id}/logs`, {
          headers: { Authorization: `Bearer ${token}` },
        }).then((r) => r.json())
      )
    );
  ...
  const saveHabit = async (data) => {
    ...
    const url = data._id
      ? `http://localhost:5003/habits/${data._id}`
      : "http://localhost:5003/habits";
    await fetch(url, {
      method,
      headers: {
  ...
  const deleteHabit = async (id) => {
    ...
    const token = localStorage.getItem("token");
    await fetch(`http://localhost:5003/habits/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    loadAll();
  ...
  const toggleToday = async (hid, done) => {
    const token = localStorage.getItem("token");
    await fetch(`http://localhost:5003/habits/${hid}/log`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const saveLog = async (date, state) => {
    ...
    await Promise.all(
      Object.entries(state).map(([hid, done]) =>
        fetch(`http://localhost:5003/habits/${hid}/log`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
