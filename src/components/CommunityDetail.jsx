...
export default function CommunityDetail({ communityId, onBack }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        // 1) Community
        let res = await fetch(`http://localhost:5003/communities/${id}`);
        if (!res.ok) throw new Error((await res.json()).error);
        const communityData = await res.json();

        // 2) Posts (always fetch, but we’ll conditionally render)
        res = await fetch(`http://localhost:5003/posts/community/${id}`);
        if (!res.ok) throw new Error((await res.json()).error);
  ...
  useEffect(() => {
    ...
    const fetchMembers = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5003/communities/${community._id}/members`, {
  ...
  const handleJoin = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `http://localhost:5003/communities/${id}/join`,
  ...
  const handleRequestToJoin = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `http://localhost:5003/communities/${id}/request`,
      { method: "POST", headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) throw new Error((await res.json()).error);
  ...
  const handleLeave = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `http://localhost:5003/communities/${id}/leave`,
      { method: "POST", headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) throw new Error((await res.json()).error);
    setCommunity(c => ({
  ...
  const handleRequest = async (userId, action) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `http://localhost:5003/communities/${id}/approve`,
        {
          method: "POST",
          headers: {
  ...
  const MemberRow = ({ member }) => {
    ...
    const handleRemove = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(
          `http://localhost:5003/communities/${community._id}/remove-member`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDeleteCommunity = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://localhost:5003/communities/${community._id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) throw new Error((await res.json()).error);
